...
SAM3's GEMM shapes (M=5184, N=4736, N=36288, K=256) are **very different
from LLM shapes** that BLAS libraries are most heavily tuned for.
Default kernel heuristics leave 2%+ performance on the table.
...
```
while true:
    agent reads last_result.json + tune_config.py
    agent proposes ONE change to tune_config.py
